...
* 获取镜像可以使用docker[image]pull命令直接从    Docker hub镜像源来下载镜像。该命令格式为：
  ...
  ![](../docker/docker_picture/下载镜像.png)


* 对于Docker镜像来说，如果不显示指定TAG，则默认   会选择latest标签，这会下载仓库中最新版本的镜像。

  ![](../docker/docker_picture/镜像最新.png)
...
* 下载镜像到本地后，可随时使用该镜像了，例如利用   该镜像创建一个容器，在其中运行bash应用，执行    打印"Hello World"命令：
  ...
  ![](../docker/docker_picture/测试镜像.png)
...
* 1.使用images命令列出镜像
  ...
  ![](../docker/docker_picture/镜像信息.png)
...
* 2.使用tag命令添加镜像标签
  ...
  ![](../docker/docker_picture/tag命令.png)
...
* 3.使用inspect命令查看详细信息
  ...
  ![](../docker/docker_picture/inspect命令.png)
...
* 4.使用history命令查看镜像历史
  ...
  ![](../docker/docker_picture/history命令.png)


  注：过长的命令会被自动截断，可以使用images子命令中的--no-trunc=true|false选项来输出完整命令。

## 搜寻镜像
很多时候有很多优秀或者收藏数较多的镜像，而我们通常需要使用docker search命令可以搜索Docker Hub官方仓库中的镜像。语法为docker search[option]keyword。
...
![](../docker/docker_picture/search命令.png)
...
* 使用docker rmi或docker image， rm命令可以删除镜像，命令格式为docker rmi IMAGE[IMAGE...]，其中IMAGE    可以为标签或ID。
  ...
  ![](../docker/docker_picture/删除镜像.png)
...
* 使用镜像ID来删除镜像
  ...
  ![](../docker/docker_picture/镜像ID删除.png)
...
* 3.清理镜像
  ...
  ![](../docker/docker_picture/清理镜像.png)
...
*	基于已有容器创建
   ...
   ![](../docker/docker_picture/创建test文件.png) 
   ...
   ![](../docker/docker_picture/容器创建镜像.png)
...
*	基于本地模板导入
  ...
  例如下载了ubuntu-16.04的模板压缩包，之后可以使用以下命令即可：
   ...
   ![](../docker/docker_picture/模板导入镜像.png)

  查看镜像：

  ![](../docker/docker_picture/查看镜像.png)
...
* 1.存出镜像
  ...
  ![](../docker/docker_picture/镜像载出.png)
...
本节主要学习了Docker镜像的push子命令。可以使用docker[image]push命令上传镜像到仓库，默认上传到Docker Hub官方仓库（需要登录）。命令格式为docker[image]push NAME[：TAG]|[REGISTRY_HOST[：REGISTRY_PORT]/]NAME[：TAG]。
